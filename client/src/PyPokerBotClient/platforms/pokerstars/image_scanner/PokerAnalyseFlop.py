...
class PokerAnalyseFlop(object):
    """
    This class analyses the flop, which contains the common cards that
    are shared by all players in order to create the best hand. The flop
    is available on the center of the table, and every turn a card is
    ...
    def __init__(self, platform, table_type, number_of_seats, flop_size):
        ...
        Poker Platform to be used, the TableType, the number of available
        seats and the maximum number of cards on the flop.
        ...
        self.flop_has_card_histogram = create_list_seats(self.number_of_seats)
        ...
        if self.flop_has_card_histogram[index] is None:
            self.flop_has_card_histogram[index] = get_histogram_from_image(
                grab_image_from_file(
                    Settings.get_flop_has_nocard_template(self.platform, self.table_type)))
    ...
    def check_if_flop_pos_is_empty(self, image_to_analyse, index):
        """
        ...
        template_flop_pos1 = \
            get_histogram_from_image(grab_image_pos_from_image(
                image_to_analyse,
                Settings.get_flopcard(self.platform, self.table_type, index),
        ...
        return res > self.get_flop_has_card_threshold()

    def get_card_in_flop_pos_is_empty(self, image_to_analyse, index):
        """
    ...
    def analyse_flop_template(self, image_to_analyse):
        """
        ...
        """
        ret = create_list_string_seats(self.number_of_seats)
        for index in range(self.flop_size):
            if self.check_if_flop_pos_is_empty(image_to_analyse, index):
                return ret
            selected_card = ''
            selected_card_res = 1000000000
            image_from_flop_card, current_flop_image = \
                self.get_card_in_flop_pos_is_empty(image_to_analyse, index)
            for current_suit in PokerTableScanner.suits:
                for current_card in PokerTableScanner.cards:
                    current_card_image = get_card_template(
                        self.platform,
                        current_card,
                        current_suit)
